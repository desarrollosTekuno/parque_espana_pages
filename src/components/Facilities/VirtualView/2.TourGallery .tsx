...
// Precarga silenciosa de las imágenes de una pestaña (para que el cambio se sienta instantáneo)
...
export default function TourGallery() {
  const { tabs } = VirtualViewContent.tourGallery;
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`flex shrink-0 cursor-pointer items-center gap-2 rounded-t-2xl px-4 py-3 text-[16px] font-semibold transition sm:gap-3 sm:px-5 sm:py-4 sm:text-[18px] lg:px-6 lg:text-[22px] lg:items-center ${
                      isActive
                        ? "border border-b-0 border-gray-200 bg-[#F2F4F7] text-[#3C3C3C]"
                        : "text-gray-500 hover:text-[#3C3C3C]"
                    }`}
                  >
                    <img src={iconSrc} alt="" className="h-5 w-5 object-contain sm:h-6 sm:w-6 lg:h-7 lg:w-7" />
                    {tab.label}
                  </button>
                );
              })}
            </div>
            {/* Fade indicador de scroll, solo en mobile */}
