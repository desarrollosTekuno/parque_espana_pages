...
// Imagen compartida para ambos parques
import MembershipsHero from "@/assets/images/Memberships/TeamFamily.webp";

// Configuraciones de Memberships para cada parque
...
export default function Hero() {
  ...
  // Selección dinámica únicamente del contenido (textos)
  const content = isParque2 ? parque2Content : parque1Content;

  const { title, subtitle, imageAlt, graphicImageAlt } = content.hero;

  return (
    <section className="relative bg-header-gradient">
      <div className="relative flex flex-col items-center h-auto sm:h-auto lg:h-[60vh] lg:min-h-95 lg:max-h-150 lg:flex-row overflow-hidden">

        {/* Imagen de fondo (única para ambos parques) */}
        <img
          src={MembershipsHero}
          alt={imageAlt}
          loading="lazy"
